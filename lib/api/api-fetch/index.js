...
async function fetch (resource, init) {
  ...
  _fetch({ request, requestObject }).then(p.resolve, p.reject)
...
async function abortFetch (p, request, responseObject) {
  ...
  // 3. This is a no-op if promise has already fulfilled.
  // TODO

  // 4. If request’s body is not null and is readable, then cancel request’s body with error.
  if (request.body != null) {
    // TODO: if readable?
    request.body.cancel(error)
  }

  // 5. If responseObject is null, then return.
